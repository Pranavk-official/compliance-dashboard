...
export const getStatusBadge = (percent: number) => {
  ...
  if (percent >= COMPLIANCE_THRESHOLDS.HIGH) return 'bg-blue-100 text-blue-700 border-blue-300';
  if (percent >= COMPLIANCE_THRESHOLDS.MEDIUM) return 'bg-yellow-100 text-yellow-700 border-yellow-300';
  return 'bg-red-100 text-red-700 border-red-300';
};

export const getStatusColor = (percent: number) => {
  if (percent >= 1) return 'text-green-600';
  if (percent >= COMPLIANCE_THRESHOLDS.HIGH) return 'text-blue-600';
  if (percent >= COMPLIANCE_THRESHOLDS.MEDIUM) return 'text-yellow-600';
  return 'text-red-600';
};
